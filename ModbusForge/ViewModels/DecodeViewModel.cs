...
namespace ModbusForge.ViewModels
{
    public partial class DecodeViewModel : ObservableObject
    {
        ...
        {
            try { return _main.IsConnected; } catch { return false; }
        }
        ...
        private async Task ReadAsync()
        {
            try
            {
                ...
                Status = $"Read {(UseTwoRegisters ? 2 : 1)} {(Area.StartsWith("Input", StringComparison.OrdinalIgnoreCase) ? "IR" : "HR")} from {Address}";
            }
            catch (Exception ex)
            {
