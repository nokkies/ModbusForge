...
namespace ModbusForge.ViewModels
{
    public partial class MainViewModel : ViewModelBase, IDisposable, IMonitoringCallbacks
    {
        ...
        public MainViewModel(ModbusTcpService clientService, ModbusServerService serverService, ILogger<MainViewModel> logger, IOptions<ServerSettings> options, ITrendLogger trendLogger, ICustomEntryService customEntryService, IConsoleLoggerService consoleLoggerService, ConnectionCoordinator connectionCoordinator, RegisterCoordinator registerCoordinator, CustomEntryCoordinator customEntryCoordinator, TrendCoordinator trendCoordinator, ConfigurationCoordinator configurationCoordinator, MonitoringCoordinator monitoringCoordinator, IUnitConfigurationStore unitConfigurationStore, IDialogService? dialogService = null, VisualNodeEditorViewModel? visualNodeEditorViewModel = null, IDispatcher? dispatcher = null, IThemeService? themeService = null, IUpdateCheckService? updateCheckService = null, ISettingsService? settingsService = null)
        {
            ...
            // Kick off an asynchronous update check on startup if enabled.
            // This is intentionally fire-and-forget; any UI is marshalled to the dispatcher.
            if (_settingsService?.CheckForUpdatesOnStartup == true && _updateCheckService != null)
            {
                _ = Task.Run(async () => await CheckForUpdatesAsync(showDialogOnlyOnUpdate: true));
            }
        ...
        private void InitializeCommands()
        {
            ...
            CheckForUpdatesCommand = new AsyncRelayCommand(async () => await CheckForUpdatesAsync(showDialogOnlyOnUpdate: false));
        }
    ...
    public partial class MainViewModel
    {
        ...
        private async Task CheckForUpdatesAsync(bool showDialogOnlyOnUpdate)
        {
            ...
            var result = await _updateCheckService.CheckForUpdateAsync(Version).ConfigureAwait(false);

            if (!string.IsNullOrEmpty(result.ErrorMessage))
            {
                if (!showDialogOnlyOnUpdate)
                {
            ...
            if (result.IsUpdateAvailable)
            {
                await _dispatcher.InvokeAsync(() =>
                {
                    var message = $"A newer version is available: {result.LatestVersion}.{Environment.NewLine}Open the release page in your browser?";
                    var choice = _dialogService.Show(
                        message,
                        "Update Available",
            ...
            else if (!showDialogOnlyOnUpdate)
            {
                await _dispatcher.InvokeAsync(() =>
                    _dialogService.Show(
