...
namespace ModbusForge.Services
{
    ...
    /// <summary>
    /// Runs a visual node simulation graph against a local Modbus data store.
    /// </summary>
    public interface IAvaloniaVisualSimulationService : IDisposable
    {
        bool IsRunning { get; }
    ...
    public class AvaloniaVisualSimulationService : IAvaloniaVisualSimulationService, IDisposable
    {
        ...
        private void EnsureGraphLoaded()
        {
            ...
            var simulationNodes = _config.Nodes.Select(MapToSimulationNode).ToList();
            var simulationConnections = (_config.Connections ?? Enumerable.Empty<NodeConnection>()).Select(MapToSimulationConnection).ToList();
